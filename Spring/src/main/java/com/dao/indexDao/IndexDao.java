...
public class IndexDao {
	...
	public static void splitword() {
		...
		String   keywordb  = new String("汽车     汽车城 全车垫        沙发    真皮    气垫     自动驾驶 福特 加长 超舒适 S500 四驱 加厚刹车");
		String[] keywordbs = keywordb.split("");
		
		System.out.println( " Split into -- " + keywordbs.length );
		
		for( int i = 0 ; i < keywordbs.length ; i++ ) {
			System.out.print( "--" + keywordbs[i] + "--");
			if(keywordbs[i].equals(""))
	...
	public static void pickhotwords() {
		...
		for ( int i = 0 ; i < keywords.length ; i++ )
			if(hotwords.contains(keywords[i]))
				System.out.println( "contains keyword." + keywords[i] );
